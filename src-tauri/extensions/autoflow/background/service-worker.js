...
function mainWorldAgentClick() {
  var btns = Array.from(document.querySelectorAll("button[aria-pressed], button[role='switch'], [role='switch']"));
  var btn = btns.find(function(b) { return /agent/i.test(b.textContent || ""); });
  if (!btn) return { ok: false, reason: "Agent button not found" };
  var pressed = btn.getAttribute("aria-pressed") || btn.getAttribute("aria-checked");
  if (pressed !== "true") return { ok: true, skipped: true };
  btn.click();
  return { ok: true };
}

// Single native click on the marked element, run in the page's MAIN world.
// Fires exactly ONE activation — used for submit when no React fiber exists.
function mainWorldDirectClick(token, markerAttr) {
  var el = document.querySelector("[" + markerAttr + '="' + token + '"]');
  if (!el) return { ok: false, reason: "element not found in MAIN world" };
  try {
    el.click();
    return { ok: true, method: "mainWorldClick" };
  } catch (e) {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg?.type === "MAIN_WORLD_DIRECT_CLICK") {
    const tabId = sender.tab?.id;
    if (!tabId) { sendResponse({ ok: false, reason: "no tab id" }); return; }
    chrome.scripting
      .executeScript({ target: { tabId }, world: "MAIN", func: mainWorldDirectClick, args: [msg.token, msg.markerAttr] })
